...
for dirPath, dirNames, fileNames in os.walk("./train_data_t/Bad"):
    for f in fileNames:
        if 'Store' not in f:
            ...
            rd = bean_train_gray.reshape((1,40000))
            feature.append(np.histogram(rd,range=(0,255),bins=256,normed=False)[0].tolist() + [0.0])
...
for dirPath, dirNames, fileNames in os.walk("./train_data_t/good"):
    for f in fileNames:
        if 'Store' not in f:
            ...
            bean_train_gray = to_gray(bean_train)
            rd = bean_train_gray.reshape((1,40000))
            feature.append(np.histogram(rd,range=(0,255),bins=256,normed=False)[0].tolist() + [1.0])
...
print(accuracy_score(Label,Ans,  normalize=True, sample_weight=None))
